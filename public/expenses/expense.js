...
const getcurrPage = async function (currpage) {
	try {
		...
		const res = await axios.get(`http://16.171.32.76:3001/expense/pagination?currpage=${currpage}&expPerPage=${noofexpperpage}`, { headers: { Authorization: token } });
...
async function paymentProcess(e) {
	try {
		...
		const response = await axios.get("http://16.171.32.76:3001/purchase/premiummembership", { headers: { Authorization: token } });
		...
		var options = {
			...
			handler: async function (response) {
				const res = await axios.post(
					"http://16.171.32.76:3001/purchase/updatetransactionstatus/success",
					{
		...
		rzp1.on("payment.failed", async function (response) {
			...
			await axios.post(
				"http://16.171.32.76:3001/purchase/updatetransactionstatus/failed",
				{
...
async function add_to_database(obj) {
	try {
		const token = localStorage.getItem("token");
		await axios.post("http://16.171.32.76:3001/expense", obj, { headers: { Authorization: token } });
		getcurrPage(localStorage.getItem("currPageOnScreen"));
	} catch (error) {
...
async function get_from_database() { 
	try {
		...
		let arr = await axios.get("http://16.171.32.76:3001/expense", { headers: { Authorization: token } });
		return arr.data;
	} catch (error) {
		console.log(error);
	}
...
async function showleaderboard() {
	try {
		...
		let token = localStorage.getItem("token");
		let newarr = await axios.get("http://16.171.32.76:3001/premium/leaderboard", { headers: { Authorization: token } });
		let arr = newarr.data;
...
async function delete_from_database(id) {
	try {
		let token = localStorage.getItem("token");
		await axios.delete(`http://16.171.32.76:3001/expense/${id}`, { headers: { Authorization: token } });
		getcurrPage(1);
	} catch (error) {
		console.log(error);
		let errorDiv = document.createElement("div");
...
async function download() {
	try {
		let token = localStorage.getItem("token");
		let res = await axios.get("http://16.171.32.76:3001/expense/download", { headers: { Authorization: token } });
...
async function show_fileURL_list() {
	try {
		let token = localStorage.getItem("token");
		let lis = await axios.get("http://16.171.32.76:3001/expense/allfiles", { headers: { Authorization: token } });
		let arr = lis.data;
